...
int main(int argc, char* argv[]) {
  ...
  command.hostname = argv[argc-1-num_of_optparams_specified];
  ...
  // Construct either a UDP or TCP socket
  int byte_type;
  ...
  // Create a send and recv buffer
  char buffer[DEFAULT_BUFFER_SIZE]; // send buffer
  char *buffer_addr;
  ...
  char *recv_buffer_addr;
  ssize_t n_bytes_to_read_recvB;
  int recv_buffer_free_space;
  buffer_addr = buffer;
  buffer_free_space = DEFAULT_BUFFER_SIZE;
  ...
  if (command.udp_flag != 0 && command.hostname != 0 && command.port != 0) {
    mux.controller = 1;
    connect( socketfd, ( struct sockaddr * ) &client_addr, sizeof( client_addr ) );
...
//    send( socketfd, buffer_addr, strlen(buffer_addr), 0);
